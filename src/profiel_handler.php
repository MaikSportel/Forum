...
//Gegevens uit het register formulier halen
    $voornaam = htmlentities($_POST['voornaan_add']);

    $add_voornaam = $dbh->prepare('INSERT INTO users (	firstname) VALUES (:voornaam) WHERE if (isset($_SESSION["ID"]))');
    $add_voornaam->execute([':voornaam' => preg_replace('/\s+/', '', $voornaam)]);
    $verander_gb_message = '<section>Je voornaam is toegevoegd!</section>';
