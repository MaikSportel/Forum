...
//            $create_topic->execute([':cat_id' => 4, ':subcat_id' => 1, ':auteur' => preg_replace('/\s+/', '', $topic_username), ':titel' => $topic_titel, ':inhoud' => $topic_inhoud]);
            $create_topic->execute(array( 4,  1, preg_replace('/\s+/', '', $topic_username), $topic_titel, $topic_inhoud));
            $topic_aangemaakt = '<section>Je topic is aangemaakt</section>';
        }
