...
//            $create_topic->execute([':cat_id' => 2, ':subcat_id' => 2, ':auteur' => preg_replace('/\s+/', '', $topic_username), ':titel' => $topic_titel, ':inhoud' => $topic_inhoud]);
            $create_topic->execute(array( 2,  2, preg_replace('/\s+/', '', $topic_username), $topic_titel, $topic_inhoud));
            $topic_aangemaakt = '<section>Je topic is aangemaakt</section>';
        }
